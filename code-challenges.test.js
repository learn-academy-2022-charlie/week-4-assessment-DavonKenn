...
expect(colorScheme(colors1)).not.toEqual(expect.arrayContaining(["purple"])) ,
expect(colorScheme(colors2)).toEqual(expect.arrayContaining(["chartreuse","indigo", "periwinkle", "ochre", "aquamarine", "saffron"])) 
expect(colorScheme(colors2)).not.toEqual(expect.arrayContaining(["chartreuse"]))
})
});
...
// // will be using two methods .splice or .shift() but .filiter has built in conditional statement.
